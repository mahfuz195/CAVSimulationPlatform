...
if [ -z "$SUMO_HOME" ]; then
  export SUMO_HOME=$WEBOTS_HOME/projects/automobile/resources/sumo
fi
...
python importer.py --input=$osm_file_path --config-file=$script_dir/resources/OSM_importer/config.ini --output=$script_dir/worlds/$1.wbt
set +o xtrace
...
$SUMO_HOME/bin/duarouter --trip-files $script_dir/worlds/$1_net/sumo.trip.xml --net-file $script_dir/worlds/$1_net/sumo.net.xml --output-file $script_dir/worlds/$1_net/sumo.rou.xml --ignore-errors true
set +o xtrace
