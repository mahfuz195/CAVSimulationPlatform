...
class Road(WebotsObject):
    ...
    def parse_tags(self):
        ...
        if Settings.has_option(settingsSection, 'texture'):
            self.texture = Settings.get(settingsSection, 'texture')
    ...
    def split_at(self, node):
        ...
        cut = (i for i, v in enumerate(a.refs) if v == node).next()
    ...
    def export(cls, f):
        ...
        for road in cls.roads:
            ...
            for i in range(road.lanes - 1):
                f.write('    %s\n' % ('FALSE' if (i == road.forwardLanes - 1) else 'TRUE'))
            f.write('  ]\n')
            ...
            f.write('  ]\n')
            if road.texture is not None:
                f.write("  texture [\n")
                f.write("    " + road.texture + "\n")
                f.write("  ]\n")
            f.write('}\n')
...
class Crossroad:
    ...
    def process_shape(self):
        ...
        for i in range(0, 60):
            for d in distData:
                ...
                if self.nodeRef == d['road'].refs[0]:
                    ...
                    assert 0, "Could not occur if the roads has been cut correctly."
        ...
        # Increase slightly the result to simplify the collision detection with the road paths later.
        self.shape = self.shape.convex_hull.buffer(0.02)
        # Remove doubled coordinates.
        self.shape = simplify_polygon(self.shape)
