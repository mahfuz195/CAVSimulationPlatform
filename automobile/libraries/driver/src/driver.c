...
typedef struct {
  ...
  // Commands
  wbu_wipers_mode wipers_mode;
  wbu_indicator_state indicator_state;
...
static double compute_output_torque() {
  ...
  wbu_car_engine_type engine = instance->car->engine_type;

  double real_rpm = instance->rpm;
  if ((instance->gear >= 0) && (real_rpm < 0))
    real_rpm = 0;
  double engine_rpm = real_rpm;
  if ((engine_rpm < instance->car->engine_min_rpm) && (engine == WBU_CAR_COMBUTSION_ENGINE || engine == WBU_CAR_PARALLEL_HYBRID_ENGINE || engine == WBU_CAR_POWER_SPLIT_HYBRID_ENGINE))
  ...
  if (engine == WBU_CAR_COMBUTSION_ENGINE || (engine == WBU_CAR_PARALLEL_HYBRID_ENGINE && real_rpm == engine_rpm))
    engine_torque += instance->car->engine_coefficients[0] + engine_rpm * instance->car->engine_coefficients[1] + engine_rpm * engine_rpm * instance->car->engine_coefficients[2];
  if (engine == WBU_CAR_POWER_SPLIT_HYBRID_ENGINE && real_rpm == engine_rpm)
    engine_torque += (1 - instance->car->hybrid_power_split_ratio) * (instance->car->engine_coefficients[0] + instance->car->hybrid_power_split_rpm * instance->car->engine_coefficients[1] + instance->car->hybrid_power_split_rpm * instance->car->hybrid_power_split_rpm * instance->car->engine_coefficients[2]);
  if (engine == WBU_CAR_ELECTRIC_ENGINE || engine == WBU_CAR_PARALLEL_HYBRID_ENGINE || engine == WBU_CAR_POWER_SPLIT_HYBRID_ENGINE) {
  ...
  // Limit torque if maximum acceleration is reached (in order to not have big jump of wheels speed when one of them temporarly do not touch the ground)
...
static void update_wheels_speed(int ms) {  // Warning speed is wrong the first two steps
  ...
  for(i=0; i<4; i++) {
    ...
    acceleration = (instance->car->speeds[i] - previous_speed[i]) / ms;
...
static void compute_rpm() {
  ...
  double average_speed = (instance->car->speeds[0] + instance->car->speeds[1] + instance->car->speeds[2] + instance->car->speeds[3]) / 4;
  // convert to rot/min
  ...
  double rpm = fabs(average_speed * gear_ratio); // multiplication by the gear_ratio because we want motor rpm speed from wheels rpm
...
static void update_slip_ratio() {
  if (instance->car->type == WBU_CAR_TRACTION || instance->car->type == WBU_CAR_FOUR_BY_FOUR) {
    ...
    instance->front_slip_ratio = instance->front_slip_ratio + real_front_ratio; // for better result a PD controller can be used here
    ...
    instance->rear_slip_ratio = instance->rear_slip_ratio + real_rear_ratio; // for better result a PD controller can be used here
    ...
    instance->central_slip_ratio = instance->central_slip_ratio + real_central_ratio; // for better result a PD controller can be used here
...
static void update_torque() {
  ...
  if (instance->car->type == WBU_CAR_TRACTION) {
    // Geometric differential ratio (left and right wheel should not have same torque because the rotation radius is not the same)
    ...
    if (instance->car->limited_slip_differential)
      ratio *= 2 * (1 - ((instance->front_slip_ratio + 1) / 2)); // the formula is just in order to keep ratio between 0 and 1
    ...
    // Add the limited differential slip if enable
    if (instance->car->limited_slip_differential)
      ratio *= 2 * (1 - ((instance->rear_slip_ratio + 1) / 2)); // the formula is just in order to keep ratio between 0 and 1
    // Compute and apply torques
    double right_torque = torque * ratio;
    double left_torque  = torque * (1 - ratio);
    ...
    // Add the limited differential slip if enable
    if (instance->car->limited_slip_differential) {
      ratio_front   *= 2 * (1 - ((instance->front_slip_ratio   + 1) / 2)); // the formula is just in order to keep ratio between 0 and 1
...
static void update_brake() {
  ...
  int i;
  for(i=0; i<4; i++) {
    // we divide by the rotational speed of the wheels because we want to remove the dependency of the damping friction to the rotational speed
    damping_constant = fabs(instance->brake * instance->car->brake_coefficient / instance->car->speeds[i]);
...
static void update_engine_sound() {
  if (strcmp(instance->car->engine_sound, "none") != 0) {
    ...
    if (instance->control_mode == TORQUE) {
      ...
      volume *= (1.0 - RPM_TO_VOLUME_GAIN) + RPM_TO_VOLUME_GAIN * (rpm > instance->car->engine_max_rpm ? instance->car->engine_max_rpm : rpm) / instance->car->engine_max_rpm;
    } else {
    ...
    if (stop_sound)
      ...
      wb_speaker_play_sound(instance->engine_speaker, instance->engine_speaker, instance->car->engine_sound, volume, pitch, 0.0, true);
  }
}
...
int wbu_driver_step() {
  ...
  // wipers
  if ((instance->car->wiper_motors[0] != 0 && instance->car->wiper_motors[1] != 0 && instance->car->wiper_sensor != 0) && instance->wipers_mode != DOWN) {
    bool stop = ((int)(wb_robot_get_time()) % 4) != 0; // allows for wipers in slow mode to be activated once every 4 seconds.
    if (wb_position_sensor_get_value(instance->car->wiper_sensor) == wb_motor_get_min_position(instance->car->wiper_motors[0])) { //wipers go up
    ...
    if (wb_position_sensor_get_value(instance->car->wiper_sensor) == wb_motor_get_max_position(instance->car->wiper_motors[0]) || (instance->wipers_mode == SLOW && stop)) { //wipers go/stay down
  ...
  if (((instance->control_mode  == TORQUE) && (instance->gear == -1)) || ((instance->control_mode  == SPEED) && (instance->cruising_speed < 0)))
...
void wbu_driver_set_cruising_speed(double speed) {
  ...
  if (instance->car->type == WBU_CAR_TRACTION || instance->car->type == WBU_CAR_FOUR_BY_FOUR) {
    ...
    if (instance->car->front_wheel_radius == -1.0) {
      ...
      if (instance->control_mode == TORQUE) { // if control in torque was previously enable we need to reset the target position to infinite
...
double wbu_driver_get_current_speed() {
  ...
  case WBU_CAR_FOUR_BY_FOUR:
    if (instance->car->front_wheel_radius == -1.0 || instance->car->rear_wheel_radius == -1.0)
      ...
      speed = 0.25 * (instance->car->speeds[0] + instance->car->speeds[1]) * instance->car->front_wheel_radius + 0.25 * (instance->car->speeds[2] + instance->car->speeds[3]) *instance->car->rear_wheel_radius;
    break;
...
void wbu_driver_set_indicator(int state) {
  if (!_wbu_car_check_initialisation("wbu_driver_init()", "wbu_driver_set_indicator()"))
    return;
  instance->indicator_state = (wbu_indicator_state)state;
...
double wbu_driver_get_rpm() {
  if (!_wbu_car_check_initialisation("wbu_driver_init()", "wbu_driver_get_rpm()"))
    return 0.0;
  if (instance->control_mode  == SPEED) {
    fprintf(stderr, "Warning: no engine model when cruise control is enable, call to 'wbu_driver_get_rpm()' is therefore not allowed.\n");
    return 0.0;
  }
  return instance->rpm;
}
...
int wbu_driver_get_gear() {
  if (!_wbu_car_check_initialisation("wbu_driver_init()", "wbu_driver_get_gear()"))
    return 0;
  if (instance->control_mode  == SPEED) {
    fprintf(stderr, "Warning: no gear model when cruise control is enable, call to 'wbu_driver_get_gear()' is therefore not allowed.\n");
    return 0;
  }
...
wbu_control_mode wbu_driver_get_control_mode() {
  if (!_wbu_car_check_initialisation("wbu_driver_init()", "wbu_driver_get_control_mode()"))
    return (wbu_control_mode)-1;
  return instance->control_mode;
}

wbu_wipers_mode wbu_driver_get_wipers_mode() {
  if (!_wbu_car_check_initialisation("wbu_driver_init()", "wbu_driver_get_wipers_mode()"))
    return (wbu_wipers_mode)-1;
  return instance->wipers_mode;
}

void wbu_driver_set_wipers_mode(int mode) {
  if (!_wbu_car_check_initialisation("wbu_driver_init()", "wbu_driver_set_wipers_mode()"))
    return;
  if (instance->car->wiper_motors[0] != 0 && instance->car->wiper_motors[1] != 0) {
    instance->wipers_mode = mode;
